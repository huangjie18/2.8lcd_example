...
#line 112 "..\\code\\ffs\\ff.c"


 
#line 125 "..\\code\\ffs\\ff.c"




 
#line 141 "..\\code\\ffs\\ff.c"
...
#line 173 "..\\code\\ffs\\ff.c"


 
#line 183 "..\\code\\ffs\\ff.c"
...
#line 231 "..\\code\\ffs\\ff.c"

#line 247 "..\\code\\ffs\\ff.c"
...
#line 1513 "..\\code\\ffs\\ff.c"
	...
	for (;;) {
		...
		if (c >= 0x80) {				 
			b |= 3;						 
#line 1552 "..\\code\\ffs\\ff.c"
		}
		if (((BYTE)(c) >= 0x81 && (BYTE)(c) <= 0xFE)) {				 
			d = (BYTE)p[si++];			 
			if (!(((BYTE)(d) >= 0x40 && (BYTE)(d) <= 0x7E) || ((BYTE)(d) >= 0x80 && (BYTE)(d) <= 0xFE)) || i >= ni - 1)	 
...
#line 2109 "..\\code\\ffs\\ff.c"
	;

	if (res == FR_OK) {
		fp->flag = mode;					 
		fp->org_clust =	(((DWORD)(WORD)(((WORD)*(BYTE*)((dir+20)+1)<<8)|(WORD)*(BYTE*)(dir+20))<<16) | (WORD)(((WORD)*(BYTE*)((dir+26)+1)<<8)|(WORD)*(BYTE*)(dir+26)));		 
...
#line 2400 "..\\code\\ffs\\ff.c"
	res = f_sync(fp);		 
#line 2414 "..\\code\\ffs\\ff.c"
		...
		if (ofs == 0xFFFFFFFF) {	 
			...
			if (cl) {
				do {
					...
					do {		 
						pcl = cl; ncl++;
						cl = get_fat(fp->fs, cl);
						if (cl <= 1) { fp->flag |= 0x80; return FR_INT_ERR; };
						if (cl == 0xFFFFFFFF) { fp->flag |= 0x80; return FR_DISK_ERR; };
					} while (cl == pcl + 1);
					*tbl++ = ncl; *tbl++ = tcl;
					tlen -= 2;
			...
			*tbl = 0;	 
			...
			if (ofs) {
				...
				for (;;) {
					ncl = *tbl++;
					if (!ncl) { fp->flag |= 0x80; return FR_INT_ERR; };
					if (cl < ncl) break;
					cl -= ncl; tbl++;
				}
				...
				if (!dsc) { fp->flag |= 0x80; return FR_INT_ERR; };
				dsc += csc;
				if (fp->fptr % 512U && dsc != fp->dsect) {
...
#line 3298 "..\\code\\ffs\\ff.c"



#line 3520 "..\\code\\ffs\\ff.c"
...
)
{
	...
	for (cc = res = 0; cc != (-1); res += cc) {
		...
		switch (d) {				 
		case 'S' :					 
			cc = f_puts(__va_arg(arp, TCHAR*), fil); continue;
		case 'C' :					 
		...
		val = (f & 2) ? __va_arg(arp, long) : ((d == 'D') ? (long)__va_arg(arp, int) : __va_arg(arp, unsigned int));
