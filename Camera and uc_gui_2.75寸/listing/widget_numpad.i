#line 1 "..\\code\\Test ucGUI\\WIDGET_NumPad.c"
...
#line 21 "..\\code\\Test ucGUI\\WIDGET_NumPad.c"
...
#line 22 "..\\code\\Test ucGUI\\WIDGET_NumPad.c"
...
static int _aKey[] = {30, 9, 16, 18};

 
static const GUI_WIDGET_CREATE_INFO _aDialogNumPad[] = {
 
  { WINDOW_CreateIndirect,   0,        0,                225, 110,  95, 130},
  { BUTTON_CreateIndirect,   "7",      0x800 +  7,   5,   5,  25,  20},
  { BUTTON_CreateIndirect,   "8",      0x800 +  8,  35,   5,  25,  20},
  { BUTTON_CreateIndirect,   "9",      0x800 +  9,  65,   5,  25,  20},
  { BUTTON_CreateIndirect,   "4",      0x800 +  4,   5,  30,  25,  20},
  { BUTTON_CreateIndirect,   "5",      0x800 +  5,  35,  30,  25,  20},
  { BUTTON_CreateIndirect,   "6",      0x800 +  6,  65,  30,  25,  20},
  { BUTTON_CreateIndirect,   "1",      0x800 +  1,   5,  55,  25,  20},
  { BUTTON_CreateIndirect,   "2",      0x800 +  2,  35,  55,  25,  20},
  { BUTTON_CreateIndirect,   "3",      0x800 +  3,  65,  55,  25,  20},
  { BUTTON_CreateIndirect,   "0",      0x800 +  0,   5,  80,  25,  20},
  { BUTTON_CreateIndirect,   ".",      0x800 + 10,  35,  80,  25,  20},
  { BUTTON_CreateIndirect,   "Del",    0x800 + 11,  65,  80,  25,  20},
  { BUTTON_CreateIndirect,   "Tab",    0x800 + 12,   5, 105,  25,  20},
  { BUTTON_CreateIndirect,   0,        0x800 + 13,  35, 105,  25,  20},
  { BUTTON_CreateIndirect,   0,        0x800 + 14,  65, 105,  25,  20},
};

 
static const GUI_WIDGET_CREATE_INFO _aDialogUser[] = {
 
  { FRAMEWIN_CreateIndirect, "Dialog", 0,                 40,  90, 140, 115, (1<<4)},
  { EDIT_CreateIndirect,     0,        0x100,      10,  10, 110,  20, 0, 12},
  { EDIT_CreateIndirect,     0,        0x101,      10,  40, 110,  20, 0, 12},
  { BUTTON_CreateIndirect,   "Ok",     1,         10,  70,  50,  20 },
  { BUTTON_CreateIndirect,   "Cancel", 2,     70,  70,  50,  20 },
};
...
static void _cbDialogNumPad(WM_MESSAGE * pMsg) {
  ...
  case 0x000F:
    ...
    break;
  case 30:
    for (i = 0; i < (sizeof(_aDialogNumPad) / sizeof(_aDialogNumPad[0])) - 1; i++) {
      hItem = WM_GetDialogItem(hDlg, 0x800 + i);
      BUTTON_SetFocussable(hItem, 0);                        
      switch (i) {
      case 13:
        BUTTON_SetBitmapEx(hItem, 0, &_bmArrowLeft, 7, 7);   
        BUTTON_SetBitmapEx(hItem, 1, &_bmArrowLeft, 7, 7);   
        break;
      case 14:
        BUTTON_SetBitmapEx(hItem, 0, &_bmArrowRight, 7, 7);  
        BUTTON_SetBitmapEx(hItem, 1, &_bmArrowRight, 7, 7);  
        break;
      }
    }
    hItem = WM_GetDialogItem(hDlg, 0x800 + 12);
    break;
  case 38:
    ...
    case 1:
      Pressed = 1;
    case 2:
      if ((Id >= 0x800) && (Id <= (0x800 + (sizeof(_aDialogNumPad) / sizeof(_aDialogNumPad[0])) - 1))) {
        int Key;
        if (Id < 0x800 + 11) {
          char acBuffer[10];
...
static void _cbDialogUser(WM_MESSAGE * pMsg) {
  ...
  case 30:
    for (i = 0; i < 2; i++) {
      hItem = WM_GetDialogItem(hDlg, 0x100 + i);   
...
static void _cbDesktop(WM_MESSAGE * pMsg) {
  ...
  case 0x000F:
    ...
    GUI_DispNextLine();
    for (i = 0; i < (sizeof(_apExplain) / sizeof(_apExplain[0])); i++) {
...
void MainTask_WIDGET_NumPad(void) {
  GUI_HWIN hNumPad;
  GUI_Init();
  WM_SetCallback(WM_GetDesktopWindow(), _cbDesktop);
  hNumPad = GUI_CreateDialogBox(_aDialogNumPad, 
                                (sizeof(_aDialogNumPad) / sizeof(_aDialogNumPad[0])), 
                                _cbDialogNumPad, WM_GetDesktopWindow(), 0, 0);  
  WM_SetStayOnTop(hNumPad, 1);
  GUI_ExecDialogBox(_aDialogUser, 
                      (sizeof(_aDialogUser) / sizeof(_aDialogUser[0])), 
                      _cbDialogUser, WM_GetDesktopWindow(), 0, 0);              
