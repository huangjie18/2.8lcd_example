...
#line 5 "..\\code\\GPS\\GPS_tiky.c"
#line 6 "..\\code\\GPS\\GPS_tiky.c"
#line 1 "..\\code\\GPS\\GPS_tiky.h"

unsigned int const sin_table[]={
...
for(a=0;(a<Char_num)&&(b<65535); )
	{
	if(USART_GetFlagStatus(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800)), ((uint16_t)0x0020)) == SET) 
		{
		*(p+a)=((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800))->DR;
...
struct
{
	...
	while(1)
	{
	((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800))->SR;
	if(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800))->DR=='$')	 
		{ 
			G_M_V=0;
			for(a=0; a<6; )
				{  
				if(USART_GetFlagStatus(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800)), ((uint16_t)0x0020)) == SET) 
					{*(gh+a)=((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800))->DR; a++;}
				if(gh[4]=='G') G_M_V=1;
				if(gh[4]=='M') G_M_V=2;
				if(gh[5]=='V') G_M_V=3;
			...
			if(G_M_V>0)
				{	
					...
					{
					if(USART_GetFlagStatus(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800)), ((uint16_t)0x0020)) == SET)
						if(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800))->DR==',')a=100;
					}while(a==6);
					comma_counter=1;
						...
						{													  
							...
							case 2:
								...
								if(G_M_V==4)GPS_Char(2,GPS.GPGSA[0]);
								comma_counter++;
								break;
							case 3:
							...
							case 4:
								...
								if(G_M_V==3)
								{
									if(GPS.GPGSV_num[0]*4+1<=GPS.GPGSV_star[0])
									{
									...
									if(GPS.GPGSV_num[0]*4+2<=GPS.GPGSV_star[0])
									{
									...
									if(GPS.GPGSV_num[0]*4+4<=GPS.GPGSV_star[0])
									{
									...
									if(GPS.GPGSV_num[0]*4+4==GPS.GPGSV_star[0])G_M_V=100;
									}
									else G_M_V=100;
									comma_counter=10;
								}
			...
			if(GPS_menu==1)
			{
				if(G_M_V==1)
				{
					...
					s[2]='\0';
					Lcd_WriteChineseStringClarity(16*3,16*5,0xFFE0,0,s,1);
					s[0]=GPS.GPS_Time[2];s[1]=GPS.GPS_Time[3];
					GUI_DispStringAt(s,16*4+8,16*5);

					s[0]=GPS.GPS_Time[4];s[1]=GPS.GPS_Time[5];					
					GUI_DispStringAt(s,16*6,16*5);
					...
					gh[3]=0xE3;
					gh[4]='\0';
					Lcd_WriteChineseStringClarity(40+8,16+16*5+2,0xFFE0,0,gh,1);
					gh[0]=GPS.Latitude[2];
					gh[1]=GPS.Latitude[3];
					gh[2]=0x27;
					gh[3]='\0';
					...
					gh[4]=0xE3;
					gh[5]='\0';
					Lcd_WriteChineseStringClarity(40,32+16*5+4,0xFFE0,0,gh,1);
					...
					Lcd_WriteChineseStringClarity(16*4+8,96+16*5+12,0xFFE0,0,GPS.Height,1);
				}
			if(G_M_V==2)
				{
				Lcd_WriteChineseStringClarity(16*4+8,112+16*5+14,0xFFE0,0,GPS.Speed,1);
				Lcd_WriteChineseStringClarity(16*4+8,128+16*5+16,0xFFE0,0,GPS.Course,1);
				if(GPS.UTC_Date[0]!='\0')
					{
						if(Local_time>15)GPS.UTC_Date[1]++;
						s[0]=GPS.UTC_Date[4];s[1]=GPS.UTC_Date[5];
						Lcd_WriteChineseStringClarity(16*13,16*5,0xFFE0,0,s,1);
						s[0]=GPS.UTC_Date[2];s[1]=GPS.UTC_Date[3];
						Lcd_WriteChineseStringClarity(16*13+24,16*5,0xFFE0,0,s,1);
						s[0]=GPS.UTC_Date[0];s[1]=GPS.UTC_Date[1];
						Lcd_WriteChineseStringClarity(16*13+48,16*5,0xFFE0,0,s,1);
					}
				}
			}
				...
				{		
					...
					{						
						...
						GUI_Context . LCD . aColorIndex16[0]=0;
						GUI_Context . LCD . aColorIndex16[1]=0xffff;
						for(a=0;a<GPS.GPGSV_star[0];a++)
						{
							if(GPS.GPGSV[a][7]!='\0')
							{
								...
								if(GPS.GPGSV[a][10])
									Lcd_ColorBox(21*a,240-ty,19,ty,0x07E0);
								else
									Lcd_ColorBox(21*a,240-ty,19,ty,0x001F);
								s[0]=GPS.GPGSV[a][0];
								s[1]=GPS.GPGSV[a][1];
								s[2]='\0'; 
								...
								GUI_DispStringAt(s,21*a+2,240-ty-16);
							}
						}
					}
					if(GPS_menu==4)
					{						
						...
						for(a=0;a<GPS.GPGSV_star[0];a++)
						{
							...
							{
								r=90-((GPS.GPGSV[a][2]-'0')*10+GPS.GPGSV[a][3]-'0');
								angle=((GPS.GPGSV[a][4]-'0')*100+(GPS.GPGSV[a][5]-'0')*10+GPS.GPGSV[a][6]-'0');
								switch(angle/90)
								{
									case 0:
								...
								if(GPS.GPGSV[a][7]=='\0')
									GUI_Context . LCD . aColorIndex16[1]=GUI_Context . LCD . aColorIndex16[0]=0xF800;
								if(GPS.GPGSV[a][7]!='\0')
									if(GPS.GPGSV[a][10])
									GUI_Context . LCD . aColorIndex16[1]=GUI_Context . LCD . aColorIndex16[0]=0x07E0;
									else GUI_Context . LCD . aColorIndex16[1]=GUI_Context . LCD . aColorIndex16[0]=0x001F;
								GUI_FillEllipse(tx, ty, 14, 8);
								if(GUI_Context . LCD . aColorIndex16[1]==0x07E0)GUI_Context . LCD . aColorIndex16[1]=0x001F;
								else GUI_Context . LCD . aColorIndex16[1]=0xFFFF;
								s[0]=GPS.GPGSV[a][0];
								s[1]=GPS.GPGSV[a][1];
								s[2]='\0'; 
								GUI_SetFont(&GUI_Font6x8);
								GUI_DispStringAt(s,tx-5,ty-3);
							}					
						}
				  }	
				}
		...
		if(GPIO_ReadInputDataBit(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)),((uint16_t)0x0080))==0)
		{
		ty=TOUCH_Y();
		tx=TOUCH_X();
		...
		if(tx>16*16&&tx<320)	
		{
			if(ty>16*8&&ty<16*8+16)
			{
				Lcd_Clear(0x0000);
				GUI_Context . LCD . aColorIndex16[1]=0xFFFF;
				GUI_Context . LCD . aColorIndex16[0]=0x0000;
				GUI_SetFont(&GUI_Font8x16);
				r=b=0;
				while(1)
				{
					if(USART_GetFlagStatus(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800)), ((uint16_t)0x0020)) == SET) 
					{
						s[0]=((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800))->DR;
						if((s[0]!='\r')&&(s[0]!='\n'))
						{
							...
							if(r<312)r+=8;
								...
								{
									if(GUI_Context . LCD . aColorIndex16[1]==0xFFFF) GUI_Context . LCD . aColorIndex16[1]=0xFFE0;
									else GUI_Context . LCD . aColorIndex16[1]=0xFFFF;
									r=b=0;
								}
						}
			...
			if(ty>16*12&&ty<16*12+16)
			{
				Lcd_Clear(0x0000);
				GUI_Context . LCD . aColorIndex16[1]=0xffff;
				GUI_Context . LCD . aColorIndex16[0]=0xffff;
				GUI_DrawCircle(124,120,119);
				GUI_DrawCircle(124,120, 60);
				GPS_menu=4;
				GPS_button(GPS_menu);			
			...
			while(GPIO_ReadInputDataBit(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)),((uint16_t)0x0080))==0);
		}
		}
	}
}
...
void GPS(void)
{
	...
	GPS_RX();
	GUI_Context . LCD . aColorIndex16[0]=0xFFFF;
	GUI_Context . LCD . aColorIndex16[1]=0xF800;
	GUI_SetFont(&GUI_Font8x16);
}
